...
/// what are the probabilities for the next character?". Using `passgen_markov_add`, you
/// can add words to it. Given a sequence of characters, using `passgen_markov_generate`
/// you can generate a new character at random.
///
/// The @ref length of a Markov chain is the length of tuples that it looks at.
typedef struct passgen_markov {
...
/// Leaf node in the Markov chain. This contains a sorted list of codepoint-count tuples.
/// During insertion, these are sorted by the codepoint. When finalizing the Markov chain,
/// they might be reordered to be sorted by the frequency to allow for more efficient
/// generation.
typedef struct passgen_markov_leaf {
    size_t total_count;
    size_t capacity;
    union {
        uint32_t count[1];
...
/// Allocates a new Markov chain node with a size determined by the @ref size_index.
///
/// Markov chain nodes only exist in specific, discrete sizes that are prime numbers.
/// This is because they implement a kind of hash map and that makes them more efficient.
/// The @ref size_index is an index into a static array of precomputed prime sizes.
...
///
/// This returns a pointer to a passgen_markov_node because the node may need to
/// be reallocated if it would otherwise overflow.
...
///
/// This will recursively insert the word into the appropriate child nodes. It needs to
/// know the @ref length of the word (this is equal to the @ref level of the Markov chain,
/// minus the depth it has already traversed) and the weight of the word.
...
/// Allocates a new Markov leaf node with a size determined by the @ref size_index.
///
/// Markov leaf nodes only exist in specific, discrete sizes that are prime numbers.
/// This is because they implement a kind of hash map that makes them more efficient.
/// The @ref size_index is an index into a static array of precomputed prime sizes.
