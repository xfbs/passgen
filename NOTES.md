...
This is implemented with `passgen_mem_accounting`, which keeps allocated memory
blocks in a linked list and is able to scream when memory issues occur. This is
portable and used in the unit tests.

### Memory release testing

>   Make sure there is no memory leaks or anything.

Partially implemented. Done using `passgen_mem_accounting` in the unit tests,
but as of now not applied to the whole binary. Can be done on-demand using
sanitizers, tho.

### [DONE] Generate pronounceable words
...
Implemented using markov chain of letters (with a length of 2). Works on
individual codepoints, so Unicode support. Data structure is optimised for fast
traversal, and autogenerated from a wordlist using a Ruby script.

>   Maybe add `\P` for uppercase or mixed-case or something?

Or implement special identifies?
