...
static inline passgen_token_t
passgen_token_error_unicode_empty(size_t start, size_t end, size_t data_start) {
     return (passgen_token_t){
...
static inline passgen_token_t passgen_token_eof(size_t start) {
     return (passgen_token_t){
...
static inline passgen_token_t passgen_token_unicode(
    size_t start,
    size_t end,
    size_t data_start,
    size_t data_end,
...
passgen_token_t
passgen_token_parse_unicode(size_t start, unicode_iter_t *iter) {
     size_t pos = iter->pos;
     ...
     while(result.ok && result.codepoint != '}') {
          ...
          if('0' <= result.codepoint && result.codepoint <= '9') {
               ...
               return passgen_token_error_unicode_hex(
                   start,
                   data_start,
                   pos,
                   iter->pos);
     ...
     if(data_len_codepoints == 0) {
          return passgen_token_error_unicode_empty(
              start,
              iter->pos,
              data_start);
     }
...
passgen_token_special(size_t start, unicode_iter_t *iter);
...
passgen_token_t passgen_token_next(unicode_iter_t *iter) {
     ...
     if(!result.ok) {
          if(result.error == 0 && result.codepoint == 0) {
               ...
               return passgen_token_error_utf8(
                   start,
                   start,
                   iter->pos,
                   result.error);
