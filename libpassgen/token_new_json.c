...
json_t *token_parser_to_json(struct token_parser *parser) {
    ...
    if(parser->state == TOKEN_UNICODE_PAYLOAD) {
        ...
        json_object_set_new(data, "length", json_integer(parser->data.unicode_payload.length));
        json_object_set_new(data, "codepoint", json_integer(parser->data.unicode_payload.codepoint));
