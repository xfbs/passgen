...
test_result test_pattern_parse_str() {
  ...
  segments =
      passgen_array_get(&pattern.group.segments, sizeof(pattern_segments_t), 0);
  assert(segments);
...
test_result test_pattern_parse_orstr() {
  ...
  assert(segment->data.character.codepoint == 'c');
  segments =
      passgen_array_get(&pattern.group.segments, sizeof(pattern_segments_t), 1);
  assert(segments);
...
test_result test_pattern_uniescape() {
  ...
  range =
      passgen_array_get(&segment->data.range.items, sizeof(pattern_range_t), 0);
  assert(range->start == 0xF6);
  assert(range->end == 0xF6);
  range =
      passgen_array_get(&segment->data.range.items, sizeof(pattern_range_t), 1);
...
test_result test_pattern_somepatt() {
  ...
  assert(range->end == 'f');
  range =
      passgen_array_get(&segment->data.range.items, sizeof(pattern_range_t), 2);
  assert(range->pos.offset == 8);
  assert(range->pos.length == 1);
  assert(range->start == 'g');
  assert(range->end == 'g');
  range =
      passgen_array_get(&segment->data.range.items, sizeof(pattern_range_t), 3);
  assert(range->pos.offset == 9);
  assert(range->pos.length == 1);
  assert(range->start == '9');
  assert(range->end == '9');
  range =
      passgen_array_get(&segment->data.range.items, sizeof(pattern_range_t), 4);
  assert(range->pos.offset == 10);
  assert(range->pos.length == 1);
  assert(range->start == '1');
  assert(range->end == '1');
  range =
      passgen_array_get(&segment->data.range.items, sizeof(pattern_range_t), 5);
