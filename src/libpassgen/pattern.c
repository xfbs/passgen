...
pattern_result_t
pattern_error_token(passgen_token_t *token, enum pattern_error kind) {
    return (pattern_result_t){
        .ok = false,
        .remove = true,
        .kind = kind,
...
pattern_parse_repeat(pattern_repeat_t *repeat, unicode_iter_t *iter);
...
void pattern_segments_free(pattern_segments_t *segments, passgen_mem_t *mem) {
    for(size_t i = 0; i < segments->items.len; i++) {
        pattern_segment_t *segment =
            passgen_array_get(&segments->items, sizeof(pattern_segment_t), i);
...
void pattern_group_free(pattern_group_t *group, passgen_mem_t *mem) {
    ...
    for(size_t i = 0; i < group->segments.len; i++) {
        pattern_segments_t *segments =
            passgen_array_get(&group->segments, sizeof(pattern_segments_t), i);
...
pattern_result_t pattern_segments_parse(
    ...
    passgen_array_init(&segment->items, sizeof(pattern_segment_t), mem);
    bool first = true;

    while(!pattern_segments_is_end(passgen_token_peek(iter))) {
        pattern_segment_t *item =
            passgen_array_push(&segment->items, sizeof(pattern_segment_t), mem);
        if(!item) {
...
pattern_result_t pattern_group_parse(
    ...
    result = pattern_group_parse_inner(group, iter, depth, mem);
    ...
    result = pattern_parse_repeat(&group->repeat, iter);
...
pattern_result_t pattern_segment_parse(
    ...
    if(pattern_group_is_start(token)) {
        if(depth == 0) {
            ...
            result =
                pattern_group_parse(&item->data.group, iter, depth - 1, mem);
        }
...
pattern_result_t pattern_char_parse(
    ...
    result = pattern_parse_repeat(&character->repeat, iter);
...
pattern_result_t pattern_ranges_parse_inner(
    ...
    passgen_array_init(&range->items, sizeof(pattern_range_t), mem);

    // parse range items until we hit end of range
    token = passgen_token_peek(iter);
    while(!pattern_ranges_is_end(token)) {
        pattern_range_t *range_item =
            passgen_array_push(&range->items, sizeof(pattern_range_t), mem);
...
pattern_result_t pattern_ranges_parse(
    ...
    result = pattern_parse_repeat(&ranges->repeat, iter);
    ...
    size_t choices = 0;
    ranges->choices_list =
        passgen_malloc(mem, sizeof(size_t) * ranges->items.len);
    for(size_t i = 0; i < ranges->items.len; i++) {
        pattern_range_t *range =
            passgen_array_get(&ranges->items, sizeof(pattern_range_t), i);
...
pattern_result_t pattern_group_parse_inner(
    ...
    passgen_array_init(&group->segments, sizeof(pattern_segments_t), mem);
    bool first = true;
    ...
    while(first || pattern_group_is_separator(passgen_token_peek(iter))) {
        ...
        pattern_result_t result =
            pattern_segments_parse(segment, iter, depth, mem);
        if(!result.ok) {
            return result;
        }
...
pattern_result_t pattern_parse(
    ...
    pattern_result_t result =
        pattern_group_parse_inner(&pattern->group, &iter, depth, mem);
    if(!result.ok) {
        return result;
    }
...
pattern_result_t pattern_special_parse(
    ...
    result = pattern_parse_sqarg(&special->arg, iter);
    ...
    /* parse length. */
    result = pattern_parse_repeat(&special->length, iter);
    ...
    /* parse repetitions. */
    result = pattern_parse_repeat(&special->repeat, iter);
...
void pattern_error_show(pattern_result_t result, const char *format) {
    printf(
        "\e[31mError at position %zu while parsing pattern: %s\n\e[0m",
        result.pos.offset,
