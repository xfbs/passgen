...
static void *passgen_malloc_accounting(void *state, size_t size) {
    ...
    struct passgen_mem_accounting_list *node = passgen_malloc(
            acc->mem,
            sizeof(struct passgen_mem_accounting_list));
...
static void *passgen_realloc_accounting(void *state, void *ptr, size_t size) {
    ...
    if(ptr) {
        ...
        if(!ptr_node) {
            passgen_mem_accounting_error(PASSGEN_MEM_ACCOUNTING_REALLOCBADPTR, ptr);
        }

        if(ptr_node->freed) {
            passgen_mem_accounting_error(PASSGEN_MEM_ACCOUNTING_REALLOCFREED, ptr_node);
        }
    }
...
#ifdef PASSGEN_DEBUG
        ptr_node->free_frames = backtrace(ptr_node->free_bt, sizeof(ptr_node->free_bt));
#endif
    }
    ...
    for(node = acc->list; node; node = node->next) {
        if(ptr == node->pointer) {
            if(node->freed) {
                ...
                passgen_mem_accounting_error(PASSGEN_MEM_ACCOUNTING_DOUBLEFREE, node);
            }
...
void
passgen_mem_accounting_cleanup(passgen_mem_accounting_t *acc) {
...
bool
passgen_mem_accounting_check(passgen_mem_accounting_t *acc) {
    struct passgen_mem_accounting_list *node;
