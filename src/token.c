...
passgen_token_t passgen_token_error(size_t pos) {
...
passgen_token_t passgen_token_eof(void) {
  return (passgen_token_t) {
    .type = PATTERN_TOKEN_EOF,
  };
}

passgen_token_t passgen_token_regular(int32_t codepoint) {
  return (passgen_token_t) {
...
passgen_token_t passgen_token_escaped(int32_t codepoint) {
  return (passgen_token_t) {
  ...
  for(size_t i = 0; i < special_chars_size; ++i) {
    if(result.codepoint == special_chars[i].chr) {
      if(special_chars[i].parser) {
        ...
        return passgen_token_escaped(special_chars[i].dest);
      }
    }
  }
...
passgen_token_t passgen_token_next(struct unicode_iter *iter) {
  struct unicode_iter_result result;

  result = unicode_iter_next(iter);
  if(!result.ok) {
    if(result.error == 0 && result.codepoint == 0) {
      return passgen_token_eof();
    } else {
      return passgen_token_error(iter->pos);
    }
  }
  ...
  if(result.codepoint != '\\') {
    return passgen_token_regular(result.codepoint);
  }

  return passgen_token_next_complex(iter);
}
